...
export class PipelineHandler {
  ...
  public static async getPipelineFromPullRequest(
    ...
    switch (ciType) {
      ...
      case CIType.GITHUB_ACTIONS:
        return this.getGithubActionsWorkflow(owner, pullRequest, ci as GitHubActionsCI, eventType);
  ...
  /**
   * Get a Github Workflow for a given pull request
   */
  private static async getGithubActionsWorkflow(
    owner: string,
    pullRequest: PullRequest,
    githubCI: GitHubActionsCI,
    eventType?: EventType
  ): Promise<Pipeline | null> {
    return githubCI.getPipeline(owner, pullRequest, PipelineStatus.RUNNING, eventType);
  }
}
